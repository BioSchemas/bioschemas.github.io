...
If your resources deal with groups of biological organisms like animals, plants, fungi or bacteria, then you may want to use the [**Taxon profile**](/profile/Taxon).

The least you need to know to describe a Taxon is its full [scientific name](https://en.wikipedia.org/wiki/Binomial_nomenclature) (i.e. including authorship and date), and its [taxonomimc rank](https://en.wikipedia.org/wiki/Taxonomic_rank) such as species, genus, family etc.
For instance, the valid scientific name of the Beluga is _Delphinapterus leucas (Pallas, 1776)_, and its rank is _species_.


You may also provide **synonyms** of the accepted/valid scientific name (property `schema:alternateName`) that keep track of the taxonomic changes and thus reflect the way biologists think of a given taxon, that is, its circumscription.
For instance, the Beluga is also called _Balaena albicans Muller, 1776_. In this respect, note that typographic elements such as parentheses, the use of uppercase vs. lowercase letters etc., are normalized by the [Codes of nomenclature](https://en.wikipedia.org/wiki/Nomenclature_codes).

It is also a grood pratice to link a Taxon to its counterparts in third-party taxonomic registers like the [NBCI taxonomy](https://www.ncbi.nlm.nih.gov/taxonomy), or data agregators like [GBIF](https://www.gbif.org/) or [Catalog of Life](https://www.catalogueoflife.org/), using the `schema:sameAs` and/or `schema:identifier` properties, so that people would know exactly what taxon you refer to.

#### TaxonName

Furthermore, some databases databases (e.g. [Zoobank](https://zoobank.org/), [IPNI](https://www.ipni.org/) and [Mycobank](https://www.mycobank.org/)) describe specifically _scientific names_ but do not keep track of how names are assigned to taxa, such that they would be poorly supported by the use of a Taxon profile. In such a case, the [**TaxonName profile**](/profile/TaxonName) is much better suited to markup the resources of these taxonomic names registries.
